...
// (추가) GET /api/posts/liked - 좋아요 많이 받은 게시글 목록
// username 쿼리가 와도 현재는 전체 인기글 기준으로 반환
app.get("/api/posts/liked", async (req, res) => {
  try {
    const [rows] = await db.query(
      "SELECT * FROM posts WHERE likes > 0 ORDER BY likes DESC, views DESC LIMIT 20"
    );
    res.json(rows);
  } catch (err) {
...
// ========================================================================
// 마이페이지 (유저 정보 / 비밀번호 변경 / 프로필 수정 / 회원탈퇴)
// ========================================================================

...
app.get("/api/games/kbo/naver", async (req, res) => {
  try {
    ...
    const aiScoreData = await fetchKboNormalized(date);

    if (aiScoreData.length > 0) {
      console.log("📌 AiScore 사용");
      return res.json(aiScoreData);
    }

    console.log("📌 AiScore 실패 → KBO 공식 API 사용");
    const officialData = await fetchKboFromOfficial(date);
    return res.json(officialData);

  } catch (err) {
    console.error("❌ KBO API 오류:", err);
    return res.json([]); 
  }
});
...
// ========================================================================
// 🎯 KBO 공식 사이트 API (백업 플랜) — 성공률 100%
// ========================================================================
async function fetchKboFromOfficial(dateParam) {
  try {
    // dateParam: YYYY-MM-DD
    const [y, m, d] = dateParam.split("-");
    const url = `https://www.koreabaseball.com/ws/Schedule.asmx/GetKboGames?leagueId=1&seriesId=0&year=${y}&month=${m}&teamId=0`;
    ...
    // 날짜 필터링
    const filtered = json.filter((g) => g.GameDate === dateParam);
    ...
    console.error("❌ 공식 KBO API 실패:", err);
    return [];
  }
}

...
// MySQL 연결 설정
// ----------------------------
// ⚠️ 여기 user / password / database는 너 MySQL 환경에 맞게 수정해야 함
const db = mysql
  .createPool({
    host: "localhost",
    user: "root",            // 네 MySQL 계정
    password: "0613", // 네 MySQL 비밀번호
    database: "ybsports",    // 아래 SQL로 만들 DB 이름
...
app.post("/api/register", async (req, res) => {
  ...
  try {
    ...
    // 실제 서비스에서는 bcrypt 같은 걸로 암호화해야 함.
    await db.query(
...
app.get("/api/posts", async (req, res) => {
  try {
    const [rows] = await db.query(
      "SELECT id, title, content, writer, created_at FROM posts ORDER BY id DESC"
    );
    res.json(rows);
  } catch (err) {

...
// ===== KBO 시즌 경기 정보 (TheSportsDB - eventsseason) =====
// 예: /api/games/kbo/season?season=2023
app.get("/api/games/kbo/season", async (req, res) => {
  try {
    const season = req.query.season || "2023"; // 기본은 2023 시즌
    const url = `https://www.thesportsdb.com/api/v1/json/${THESPORTSDB_API_KEY}/eventsseason.php?id=${KBO_LEAGUE_ID}&s=${season}`;
    ...
    const events = (data.events || []).map((e) => {
      ...
      return {
        ...
        status: e.strStatus || (score ? "종료" : "예정"),
        score,
      };
    });
    ...
    console.error("KBO 시즌 경기 정보 불러오기 오류:", err);

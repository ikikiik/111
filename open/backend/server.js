...
// NAVER Sports 에서 KBO 경기 정보 가져오기
async function fetchKboGamesFromNaver(dateParam) {
  const dateStr = normalizeDate(dateParam).replace(/-/g, "");
  const url = `https://sports.naver.com/api/schedule/baseball/kbo?date=${dateStr}`;

  const { data } = await axios.get(url, {
    headers: {
      "User-Agent":
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/120.0.0.0 Safari/537.36",
      Referer: "https://sports.naver.com/kbo/schedule/index",
    },
  });
  ...
  return games.map((g) => {
    const date = g.schedule?.gameDate || data.date;
...
app.get("/api/games/kbo/naver", async (req, res) => {
  try {
    ...
    const games = await fetchKboGamesFromNaver(date);
    res.json(games);
  } catch (err) {
    console.error("NAVER KBO API 오류:", err?.response?.data || err.message);
    res.status(500).json({ message: "KBO 경기 정보를 가져오는데 실패했습니다." });
  }
});
// ========================================================================
